...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Total Visits', value: '12,482', change: '+12%', icon: Eye, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Content Items', value: '84', change: '+3', icon: FileText, color: 'text-indigo-600', bg: 'bg-indigo-50' },
    { label: 'Active Users', value: '1,204', change: '+18%', icon: Users, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Conversion Rate', value: '3.2%', change: '+0.4%', icon: TrendingUp, color: 'text-amber-600', bg: 'bg-amber-50' },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Welcome back, {user?.email?.split('@')[0]}!</h1>
        <p className="text-slate-500 text-sm">Here's what's happening with your website today.</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm hover:border-indigo-200 hover:shadow-md transition-all group">
            <div className="flex items-start justify-between mb-4">
              <div className={`p-3 rounded-xl ${stat.bg} ${stat.color} group-hover:scale-110 transition-transform`}>
                <stat.icon className="w-5 h-5" />
              </div>
              <span className="text-xs font-medium text-emerald-600 flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded-full border border-emerald-100">
                {stat.change}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {recentActivity.map((activity, i) => (
              <div key={i} className="p-6 flex items-center gap-4 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 font-medium">by {activity.user} • {activity.time}</p>
                </div>
        ...
        <div className="bg-indigo-600 rounded-3xl p-8 text-white relative overflow-hidden group shadow-xl shadow-indigo-200">
          <div className="relative z-10">
            ...
            <button className="bg-white text-indigo-600 px-6 py-3 rounded-xl font-semibold text-sm hover:bg-indigo-50 transition-all shadow-lg">
              Documentation
            </button>
          </div>
