...
export function Navbar() {
  ...
  return (
    ...
    >
      <nav className={`flex items-center justify-between w-full max-w-7xl px-6 py-3 transition-all duration-300 rounded-full border shadow-[0_8px_32px_rgba(0,0,0,0.5)] ${
        isScrolled 
        ...
        <Link href={getLocalizedLink("/")} className="flex items-center gap-2 group">
          ...
          <span className={`font-heading text-xl font-bold tracking-tight bg-clip-text text-transparent transition-all duration-300 ${
            isScrolled 
        ...
        <div className={`hidden md:flex items-center gap-8 text-sm font-medium transition-colors duration-300 ${
          ...
          <Link href={getLocalizedLink("#how-it-works")} className={`transition-colors relative group ${isScrolled ? "hover:text-indigo-600" : "hover:text-cyan-400"}`}>
            {t.navbar.howItWorks}
            <span className={`absolute -bottom-1 left-0 w-0 h-0.5 transition-all group-hover:w-full ${isScrolled ? "bg-indigo-600" : "bg-cyan-400"}`} />
          </Link>
          <Link href={getLocalizedLink("#pricing")} className={`transition-colors relative group ${isScrolled ? "hover:text-indigo-600" : "hover:text-cyan-400"}`}>
            {t.navbar.pricing}
            <span className={`absolute -bottom-1 left-0 w-0 h-0.5 transition-all group-hover:w-full ${isScrolled ? "bg-indigo-600" : "bg-cyan-400"}`} />
          </Link>
          <Link href={getLocalizedLink("#faq")} className={`transition-colors relative group ${isScrolled ? "hover:text-indigo-600" : "hover:text-cyan-400"}`}>
            {t.navbar.faq}
            <span className={`absolute -bottom-1 left-0 w-0 h-0.5 transition-all group-hover:w-full ${isScrolled ? "bg-indigo-600" : "bg-cyan-400"}`} />
          </Link>
        </div>
          ...
          <div className="flex items-center gap-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className={`rounded-full px-3 font-bold text-xs uppercase transition-all duration-300 border ${
                  isScrolled 
              ...
              <DropdownMenuContent align="end" className={`min-w-[8rem] rounded-xl p-1 shadow-2xl backdrop-blur-lg border transition-all duration-300 ${
                isScrolled 
                  ...
                  <DropdownMenuItem 
                    ...
                    className={`rounded-lg cursor-pointer transition-colors ${
                      locale === loc 
                        ...
                        : (isScrolled ? "text-slate-600 hover:bg-slate-50 hover:text-indigo-600" : "text-zinc-400 hover:bg-white/5 hover:text-zinc-100")
                    }`}
                  >
            ...
            <Button variant="ghost" size="sm" className={`hidden sm:flex rounded-full transition-colors duration-300 ${
              isScrolled 
